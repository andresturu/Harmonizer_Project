...
//   // range of samples from which to loop forever
//   static int start_loop = 8000;
//   static int end_loop = 24000;
...
//   index += sample_count;
//   // check ahead to see no values after end_loop will be used
//   if (index + sample_count > end_loop) {
//     index = start_loop;
//   }
...
// void processAudio(int index) {
//   const NoteSample* note_ptr = &note_table[0];
//   // fun fact . and -> are NOT the same
//   // thing before . has to the OBJECT itself
//   // object.attribute or (*ptr_to_object).attribute
//   // thing before -> has to the the POINTER to the obejct, -> is shortcut for dereferencing pointer
//   // ptr_to_object->attribute

//   for (int i = 0; i < 512; i++) {
//     sample_buffer[i] = note_table[0].samples[i+index];
//     //sample_buffer[i] = (&note_table[0])->samples[i+index];
//   }
